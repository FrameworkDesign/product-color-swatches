                ...
                <div class="card p-0 relative">
                    ...
                    >
                        ...
                        <template slot="cell-source" slot-scope="{ row: redirect }">
                            <a style="word-break: break-all" :href="cp_url('weareframework/product-color-swatches/' + redirect.id)">{{
                                    redirect.source
                                }}</a>
                        </template>
                        <template slot="actions" slot-scope="{ row: redirect, index }">
                            <dropdown-list>
                                <dropdown-item
                                    :text="__('Edit')"
                                    :redirect="cp_url('weareframework/product-color-swatches/' + redirect.id)"
                                />
                                <dropdown-item
                                    :text="__('Delete')"
                                    class="warning"
...
export default {
    ...
    methods: {
        removeRow(row) {
            let id = row.id;
            let i = this.items.find((element) => element.id === id);
